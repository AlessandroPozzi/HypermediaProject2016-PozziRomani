<?php
 ...
 forDevice1_Target();
 /**
    *  Builds an instance of a multiple topic device page related to the smart life given as
	   ...
	   $mysqli = new mysqli("localhost", "timhypermediaproject2016", "", "my_timhypermediaproject2016");
         ...
         if (mysqli_connect_errno()){
            ...
            exit();
             ...
             // query of the other devices related to the same smart life
              $query_category ="SELECT d.name FROM device d JOIN for_device_1 fd JOIN sl s WHERE s.name="."'".$_GET['slX']."' AND s.id_sl = fd.id_sl AND
			 ...
			 if($result->num_rows == 1){
				...
				$head_node = $xpathsearch->query('//head'); //cat info
                 ...
                 //handle title
                    ...
                    $codeInfoCat = "<meta  id='category_info' content='sl_relation'></meta>"; //cat info
                    $codeInfoSL = "<meta  id='sl_info' content='".$_GET['slX']."'></meta>"; //related smart life info
                    //query of the img
                    ...
                    $slinfo = $doc->createDocumentFragment(); //related smart life info
                    ...
                    $slinfo->appendXML($codeInfoSL); //related smart life info
                      
                    
                    $head_node->item(0)->appendChild($catinfo);//cat info
                    $head_node->item(0)->appendChild($slinfo); //related smart life info
         ...
         if($_GET['catX'] != null){
           ...
           $backcode = "<a class='nav-link active' id='back' href='http://timhypermediaproject2016.altervista.org/php/For_Device1.php?slX=".$_GET['slX'].htmlspecialchars("&")."catX=".$_GET['catX'].htmlspecialchars("&")."orientation=".$_GET['orientation'].htmlspecialchars("&")."default=false'>Vai a ".$_GET['slX']." - Prodotti associati</a>";
